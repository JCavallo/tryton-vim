...
syn match   pythonDefStatement	/^\s*\%(def \|class \)/
  \ nextgroup=trytonFunction skipwhite
syn region  pythonFunctionFold	start="^\z(\s*\)\%(def\|class\)\>"
  \ end="\ze\%(\s*\n\)\+\%(\z1\s\)\@!." fold transparent

syn keyword trytonFieldData     on_change on_change_with depends domain ondelete
syn keyword trytonFieldData     states required select setter getter searcher loading
syn keyword trytonFieldData     digits size order_field selection_change_with schema_model

syn match   trytonFieldName     "^    [a-zA-Z_][a-zA-Z0-9][a-zA-Z0-9_]* " contained
...
syn keyword trytonFieldClass    Numeric Boolean
syn keyword trytonFieldClass    StateView StateTransition StateAction Button Wizard
syn keyword trytonKeywords      fields model Transaction Pool context PoolMeta

syn match   pythonFunction	"[a-zA-Z_][a-zA-Z0-9_]*" contained
syn match   trytonFunction      "\%(\%(on_change_with_\|on_change_\|default_\|transition_\|__setup__\|__register__\)[a-zA-Z0-9_]*\|\)" contained nextgroup=pythonFunction
syn region  trytonFieldDeclaration   start="^\z(\s*\)\([a-zA-Z_][a-zA-Z0-9_]*\) = \%(fields\.\|StateTransition\|StateView\|StateAction\)"
  \ end="\ze\%(\s*\n\)\+\%(\z1\s\)\@!." fold transparent contains=ALLBUT,pythonFunction,trytonFunction
...
if version >= 508 || !exists("did_python_syn_inits")
  ...
  HiLink pythonStatement	Conditional
  HiLink pythonDefStatement	Statement
  HiLink pythonFunction		Function
  HiLink pythonConditional	Conditional
  HiLink pythonRepeat		Conditional
  ...
  HiLink trytonFunction         Directory
  if exists("python_highlight_numbers")
    HiLink pythonNumber	Boolean
  endif
  if exists("python_highlight_builtins")
    HiLink pythonBuiltin	PreCondit
  endif
  if exists("python_highlight_exceptions")
